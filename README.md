...
本桌面包会在首次启动时自动安装依赖并启动后端，随后打开本地 Web UI。如未自动打开，请手动访问 http://localhost:5173。

### Skill 调用说明

- `skills/music-search`（`web.search`）负责联网参考，默认先给 LLM 离线资料，再按需检索未知歌曲/风格关键词
- `skills/instruments`（`audio.render`）负责把编曲方案转成离线合成音轨
- 编曲阶段由 LLM 决定是否需要联网检索；网络不可用时会回退到本地模板
